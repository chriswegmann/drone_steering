...
function getCoordinatesVideo(videoId) {
  ...
  var snapshotsPerGesture =
    seqLength.options[seqLength.selectedIndex].value / interval;
  var steeringType = document.getElementById("posture").checked
    ? "posture"
    : "gesture";
...
function getCoordSnapshot(
  sourceId,
  sourceType,
  modelType = "none",
  steeringType = "posture",
  snapshotsPerGesture = 1
) {
  ...
  estimator(sourceId, imageScaleFactor, flipHorizontal, outputStride).then(
    resp => {
      if (sourceType == "video") {
        var sample = [];
        ...
        for (i = 0; i < 17; i++) {
          if (document.getElementById("keypoint_" + i).checked) {
            ...
            document.getElementById("keypoint_" + i + "_x").innerHTML =
              sample[j - 1];
            j = sample.push(Math.round(resp.keypoints[i].position.y, 0) / 800);
            document.getElementById("keypoint_" + i + "_y").innerHTML =
              sample[j - 1];
          }
        }
        ...
        if (steeringType == "gesture") {
          ...
          if (gestureSnapshots.length > snapshotsPerGesture) {
            all_samples_json =
              all_samples_json + JSON.stringify(gestureSnapshots);
          }
        }
      }
      ...
      if (sourceType == "webcam") {
        ...
        if (modelType == "deltas") {
          ...
          document.getElementById("label_display").src =
            "../images/" + direction;
        ...
        document.getElementById("leftShoulder_x").innerHTML =
          leftShoulder_x / 800;
        document.getElementById("leftShoulder_y").innerHTML =
          leftShoulder_y / 800;
        document.getElementById("rightShoulder_x").innerHTML =
          rightShoulder_x / 800;
        document.getElementById("rightShoulder_y").innerHTML =
          rightShoulder_y / 800;
...
function loadVideos() {
  if (document.getElementById("all_videos").checked) {
    ...
    var videoId = String(
      selectedVideo.options[selectedVideo.selectedIndex].value
    );
...
function getFeatures() {
  ...
  if (document.getElementById("gesture").checked) {
    ...
    for (k = 0; k < snapshotsPerGesture; k++) {
      for (i = 0; i < 17; i++) {
        if (document.getElementById("keypoint_" + i).checked) {
          header =
            header + part[i] + "_x_" + k + "," + part[i] + "_y_" + k + ",";
        }
      }
    }
  } else {
  ...
  all_samples_clipboard_display =
    header + "label<br>" + all_samples_clipboard_display;
  ...
  // show data on page
  document.getElementById("clipboard_message").innerHTML =
    "Data copied to clipboard. Please copy it into a text file and save it as:";
  document.getElementById("clipboard_filename").innerHTML = getFileName();
  document.getElementById(
    "all_samples_clipboard_display"
  ).innerHTML = all_samples_clipboard_display;
}
...
function getFileName() {
  ...
  fileName +=
    "steptime" +
    String(stepTime.options[stepTime.selectedIndex].value) +
    "_checksum" +
    String(getPartsChecksum());

  if (document.getElementById("gesture").checked) {
    var seqLength = document.getElementById("seq_length");
    fileName +=
      "_seqlength" + String(seqLength.options[seqLength.selectedIndex].value);
  }
